...
#define WINDOWS_PE_MAGIC_PE          0x4550
#define WINDOWS_PE_MAGIC_MZ          0x4d5a
#define WINDOWS_PE_MAX_DLL_NAME      256
#define WINDOWS_PE_MAX_FUNCTION_NAME 512
#define WINDOWS_PE_MODE_UNSET        0
#define WINDOWS_PE_MODE_X86          1
#define WINDOWS_PE_MODE_X86_64       2
#define WINDOWS_PE_MAX_SECTIONS      32
#define WINDOWS_PE_SECTION_NAME_SIZE 8
#define WINDOWS_PE_MAX_DIRECTORIES   16
...
class Pe {
    ...
    public:
        ...
        int mode                       = WINDOWS_PE_MODE_UNSET;
        struct Section sections[WINDOWS_PE_MAX_SECTIONS];
        Pe(){
            for (int i = 0; i < WINDOWS_PE_MAX_SECTIONS; i++){
                sections[i].size = 0;
                sections[i].offset = 0;
        ...
        bool Setup(int input_mode){
            ...
            switch(input_mode){
                case WINDOWS_PE_MODE_X86:
                    mode = PE_MODE_X86;
                    break;
                case WINDOWS_PE_MODE_X86_64:
                    mode = WINDOWS_PE_MODE_X86_64;
                    break;
                default:
                    fprintf(stderr, "[x] unsupported elf executable mode\n");
                    mode = WINDOWS_PE_MODE_UNSET;
                    return false;
            }
            return true;
        }
        bool ReadFile(char *file_path){
            ...
            if (mode == WINDOWS_PE_MODE_X86 && coff_header->Machine != IMAGE_FILE_MACHINE_I386){
                fprintf(stderr, "[x] %s is not a valid x86 pe file\n", file_path);
                return false;
            }
            if (mode == WINDOWS_PE_MODE_X86_64 && coff_header->Machine != IMAGE_FILE_MACHINE_AMD64){
